#--------------------------------------------------------------
# optimal power flow with transmission switching
# uts= switching indicator of tranmission line
# This model is used with the script suc.run
# Developed by Cong Liu liuc@anl.gov, Argonne National Lab
#--------------------------------------------------------------
...
#constcsu2 {i in I, t in T}: csu[i,t] >= SUCC[i]*(y[i,t] - sum{n in 2..(MDT[i]+nCOLD[i]): n <= t-1} u[i, t-n] - max(0, 1-t-INOFF[i] + MDT[i]+nCOLD[i])); # Cold start cost
...
var cpg{g in G, t in T} = NLC[g]*u[g,t] + sum{k in K} MC[g,k]*delta[g,k,t];
...
gen_lmt1 {g in G, t in T}: p[g,t]=PMIN[g]*u[g,t] + sum{k in K} delta[g,k,t];
gen_lmt2 {g in G, t in T}: p[g,t] <= p_ur[g,t];
gen_lmt3 {g in G, t in T}: -p[g,t] <= -p_dr[g,t];
gen_lmt4 {g in G, t in T}: p_ur[g,t] <= PMAX[g]*u[g,t] ;
gen_lmt5 {g in G, t in T}: -p_dr[g,t] <= -PMIN[g]*u[g,t];

# supply=demand
sup_demand {t in T}: sum{g in G}p[g,t] = sum{n in N} p_bus_load[n,t];
...
flow_bal {n in N, t in T}:
   sum {g in G: BUS_UNIT[g]=n} p[g,t] - p_bus_load[n,t] - (sum{l in L: branch_from[l]=n} ff[l,t] +sum{l in L: branch_to[l]=n} fb[l,t]) + (sum{l in L: branch_to[l]=n} ff[l,t]+sum{l in L: branch_from[l]=n} fb[l,t])= 0;

##ramping rate*UC
#p[g,1] can be fixed to initial power output if provided
ramp_up {g in G, t in T: t>=2}: p_ur[g,t] - p[g,t-1] <= RU[g]*u[g,t-1] + RU[g]*y[g,t]; # use ramping up rate as start up rate temporarily
ramp_down {g in G, t in T: t>=2}: p[g,t-1] - p_dr[g,t] <= RD[g]*u[g,t]+ RD[g]*z[g,t]; # use ramping down rate as shut down rate temporarily

# Min up and startup*UC
min_up1 {g in G, t in 1..MBON[g]}: u[g,t] = 1; # initially must be on for MBON hours
min_up2 {g in G, tp in T, t in tp..tp+MUT[g]-1: tp >= MBON[g] && t <= card(T)}: y[g,tp]<=u[g,t]; # this can be replaced by aggregated constraints
#min_up2 {g in G, tp in T: tp >= MBON[g]+1 && tp <= card(T)-MUT[g]+1}: sum{t in tp..tp+MUT[g]-1} -u[g, t] <= -MUT[g]*y[g, tp];
#min_up3 {g in G, tp in T: tp >= (card(T)-MUT[g]+2) && tp <= card(T)}: sum{t in tp..card(T)} (-u[g, t] + y[g, tp]) <= 0;

# Min down and shutdown*UC
min_down1 {g in G, t in 1..MBOFF[g]}: u[g,t] = 0; # initially must be off for MBOFF hours
min_down2 {g in G, tp in T, t in tp..(tp+MDT[g]-1): tp >= MBON[g] && t <= card(T)}: z[g,tp] <= 1- u[g,t] ;
#min_down2 {g in G, tp in T: tp >= MBOFF[g]+1 && tp <= card(T)-MDT[g]+1}: sum{t in tp..tp+MDT[g]-1} (-1 + u[g, t]) <= -MDT[g]*z[g, tp];
#min_down3 {g in G, tp in T: tp >= (card(T)-MDT[g]+2) && tp <= card(T)}: sum{t in tp..card(T)} (-1 + u[g, t] + z[g, tp]) <= 0;
...
# Both
urelation {g in G, t in T: t>=2}: u[g,t] - u[g,t-1] = y[g,t]- z[g,t]; 
urelation1 {g in G}: u[g,1] - UO[g] = y[g,1]- z[g,1]; 
