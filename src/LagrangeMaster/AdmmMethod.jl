...
mutable struct AdmmMaster <: AbstractLagrangeMaster
    ...
    function AdmmMaster(;alg=1, ρ=1.0, ϵ=1e-6, maxiter=1000, maxtime=3600.0, update_interval = 1, τ = 2.0, ξ = 10.0, μ = 1.0)
        am = new()
        am.alg = alg
        am.num_vars = 0
        am.num_functions = 0
...
function run!(method::AdmmMaster)
    ...
    while method.iter < method.maxiter
        ...
        push!(method.v_list, copy(method.v))
        push!(method.λ_list, copy(method.λ))
        if (method.alg == 2)
